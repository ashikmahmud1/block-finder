...
function mergePaths(top_to_bottom_paths, bottom_to_top_paths) {
    let paths_to_merge = [];
    let table_length = game_table.length;

    for (let i = 0; i < bottom_to_top_paths.length; i++) {
        if (bottom_to_top_paths[i].length < table_length) {
    ...
    if (paths_to_merge.length !== 0) {
        ...
        for (let i = 0; i < paths_to_merge.length; i++) {
            ...
            if (!pathExist(merged_paths, new_paths) && new_paths.length === table_length)
                merged_paths.push(new_paths);
        }
        // remove merged paths from the bottom_to_top_paths
        bottom_to_top_paths = bottom_to_top_paths.filter(path => path.length === table_length);
...
function findPaths() {
    ...
    // if table length is 4*4 then in every paths there will be 4 cell so total paths will be 4
    // if table length is 5*5 then in every paths there will be 5 cell so total paths will be 5
    // if table length is 6*6 then in every paths there will be 6 cell so total paths will be 6
    // if table length is 7*7 then in every paths there will be 7 cell so total paths will be 7
    // if table length is 8*8 then in every paths there will be 8 cell so total paths will be 8
    // if table length is 9*9 then in every paths there will be 9 cell so total paths will be 9
    findBottomTopPaths();
...
function initializeTable() {
    ...
    game_table = createChunk(arr, grid_size);
    updateTable();
}
